...
> **权威源码**：`supdger/sand-workflow` 是 SandWorkflow 的唯一权威仓库。消费工作区的安装副本只用于受控同步与真实验收，不能作为并行开发源。`supdger/sandadmin` 是纯净通用宿主，只维护统一插件目录。迁移记录见 [SOURCE_MIGRATION.md](SOURCE_MIGRATION.md)。
